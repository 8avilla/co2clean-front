...
import { CarbonFootprintListData } from '@/components/CarbonFootprint/types';
...
export default function CarbonFootprintResultPage() {
  ...
  const [data, setData] = useState<CarbonFootprintListData | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (isInitialized && !hasPermission(PermissionCode.VIEW_CARBON_FOOTPRINT_ANALYSIS)) {
      toast.error('Acceso denegado', { description: 'No tienes permisos para acceder a esta sección.' });
      router.replace('/');
    }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const footprints = await CarbonFootprintService.getCarbonFootprints();
        const found = footprints.find(f => f.id === id);
        if (found) {
          setData(found);
        } else {
          toast.error('No se encontró el registro de análisis');
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white p-8 rounded-2xl border border-zinc-100 shadow-sm space-y-12">
          <div className="border-b border-zinc-100 pb-4">
            <h2 className="text-lg font-black text-zinc-900 uppercase">Análisis Desglosado por Categorías</h2>
            <p className="text-sm text-zinc-500 italic">Desglose técnico de emisiones directas e indirectas según el inventario de GEI.</p>
          </div>
          ...
          <CarbonFootprintCategorySection 
            ...
            description={`El total de tCO2e generadas en la categoría de emisiones indirectas por productos utilizados por la organización fue 334,71 tCO2e, que representan el 0,45% del total del inventario de GEI. La principal fuente emisión asociada a esta categoría fue la eliminación de desechos con 180,83 tCO2e.`}
            chartData={[
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white p-5 rounded-2xl border border-zinc-100 shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Total Emisiones</p>
              <p className="text-xl font-black text-zinc-900">73,807.25 <span className="text-xs font-normal text-zinc-500">tCO2e</span></p>
            </div>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-zinc-100 shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Mayor Fuente</p>
          ...
          <div className="bg-white p-5 rounded-2xl border border-zinc-100 shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Sede Analizada</p>
